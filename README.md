...
holes on the Mac — useful when the work laptop won’t let you install a real
...
2. Open `SkeletonKey.app` on the Mac. Grant **Accessibility** and **Input
   Monitoring** (Input Monitoring has no prompt — flip it on under
   Privacy & Security if keys don’t forward).
